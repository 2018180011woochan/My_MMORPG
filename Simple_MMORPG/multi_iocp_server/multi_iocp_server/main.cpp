...
void Dispatch()
{
	while (true) {
		...
		case OP_ACCEPT: {
			...
			AcceptEx(GSocket, c_socket, ex_over->_send_buf, 0, addr_size + 16, addr_size + 16, 0, &ex_over->_over);
			break;
		}
		case OP_RECV: {
...
int main()
{
	...
	bind(GSocket, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr));
	...
	AcceptEx(GSocket, c_socket, a_over._send_buf, 0, addr_size + 16, addr_size + 16, 0, &a_over._over);
